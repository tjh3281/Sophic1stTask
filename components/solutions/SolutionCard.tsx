...
export function SolutionCard({ solution }: { solution: Solution }) {
  ...
  return (
    <Link
      ...
      // link's name. The text stays in the document and is still read as text;
      // it just stops being the label.
    ...
    >
      {image && (
        ...
        <div className="solution-card-photo relative overflow-hidden">
          <Image
            ...
            alt=""
            fill
            // A tenth of the width it used to occupy, so the browser can fetch
            // a far smaller file than the full-bleed version needed.
            sizes="(min-width: 640px) 152px, 104px"
            className={cn(
              "object-cover saturate-[0.85]",
              // Leans toward the type on hover rather than growing the card —
              // moving the hover target itself makes the pointer flicker in and
              // out of it. One transform, and nothing reflows.
      ...
      <div className="flex flex-col p-5">
        ...
        {/* Follows the description immediately. Pinning it to the foot of the
            card lined the lists up across a row, but the rows stretch to their
            tallest card — so on a category with one machine it opened a hole in
            the middle of the card. Trailing space under a short list reads as
            breathing room; a gap in the middle reads as a bug. */}
        <div className="mt-5">
